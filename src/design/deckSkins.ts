...
const BASE = (extra: string, C: string) =>
  // radius matches the app's card/cell token so skins sit in the grid
  // exactly like the default face. background-origin:border-box comes
  // LAST (after `extra`'s background shorthand, which would reset it):
  // gradient backgrounds are otherwise sized to the padding box and
  // background-repeat wraps the tile under the 1px border — putting the
  // gradient's opposite (often near-white) edge in the border strip,
  // which reads as a hairline white seam when zoomed. (overflow:hidden
  // additionally clips child layers at the padding box, so full-bleed
  // layer art stops 1px short of the edge — skins whose foreground
  // layers touch an edge must end the wrap background in that layer's
  // color so the strip under the border matches.)
...
const BUILDERS: Record<string, Builder> = {
  ...
  D70c: ({ G, R }) => ({ extra: 'border:1px solid var(--hairline);background:linear-gradient(#cdeac0,#8fd694 96%,#3f8f57)', layers: [L('position:absolute;left:-14cqw;right:-14cqw;bottom:-8cqh;height:34cqh;border-radius:50%;background:#5aa469', ''), L('position:absolute;left:-6cqw;right:-30cqw;bottom:-14cqh;height:30cqh;border-radius:50%;background:#3f8f57', ''), L('position:absolute;top:7cqh;left:9cqw;font:700 28cqh var(--font-body);color:#1b4332', R), L('position:absolute;top:8cqh;right:9cqw;font-size:22cqh;color:#1b4332', G)] }),
  // ── refinements ───────────────────────────────────────────────────
