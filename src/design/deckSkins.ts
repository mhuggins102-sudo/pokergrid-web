...
const DESKTOP: Record<string, Builder> = {
  ...
  // Ticket: the ORIGINAL perforated treatment — a fine dashed border all
  // the way around (the ticket's torn edge) + the two punch notches. The
  // consolidation pass had swapped it for heavy dashed segments on the
  // top/bottom edges only, which read as new gray rules, not a ticket.
...
const MOBILE: Record<string, Builder> = {
  ...
  // Corner split (phone): the jumbo pip keeps its size but tucks into
  // the bottom-right, and the rank gets a clean suit-colored top-left —
  // the centered pip + blend-mode rank collided on wide ranks ("10").
  D16: ({ G, R }) => ({ extra: 'border:1px solid var(--hairline)', layers: [L('position:absolute;bottom:-8cqh;right:-6cqw;font-size:80cqh;line-height:1;opacity:.9', G), L('position:absolute;top:6cqh;left:8cqw;font:700 38cqh var(--font-body);line-height:1;letter-spacing:-.03em', R)] }),
  // Offset rings (phone): the emboss ring drops to the bottom-right at
  // near-full size (pip centered inside), freeing a clean top-left for
  // the rank — centered rings collided with wide ranks ("10").
  D42: ({ G, R }) => ({ extra: 'border:1px solid var(--hairline)', layers: [L('position:absolute;right:3cqw;bottom:3cqh;width:58cqmin;height:58cqmin;border-radius:50%;box-shadow:inset 0 2cqmin 3cqmin rgba(255,255,255,.5),inset 0 -2cqmin 3cqmin rgba(0,0,0,.12);display:flex;align-items:center;justify-content:center;font-size:32cqh;line-height:1', G), L('position:absolute;top:6cqh;left:8cqw;font:700 38cqh var(--font-body);line-height:1;letter-spacing:-.03em', R)] }),
  D42a: ({ C, G, R }) => ({ extra: 'border:1px solid var(--hairline)', layers: [L('position:absolute;right:2cqw;bottom:2cqh;width:62cqmin;height:62cqmin;border-radius:50%;box-shadow:inset 0 2cqmin 3cqmin rgba(255,255,255,.5),inset 0 -2cqmin 3cqmin rgba(0,0,0,.12)', ''), L(`position:absolute;right:calc(2cqw + 9cqmin);bottom:calc(2cqh + 9cqmin);width:44cqmin;height:44cqmin;border-radius:50%;border:1cqmin solid ${C};opacity:.4`, ''), L('position:absolute;right:2cqw;bottom:2cqh;width:62cqmin;height:62cqmin;display:flex;align-items:center;justify-content:center;font-size:28cqh;line-height:1', G), L('position:absolute;top:6cqh;left:8cqw;font:700 36cqh var(--font-body);line-height:1;letter-spacing:-.03em', R)] }),
