...
const DESKTOP: Record<string, Builder> = {
  ...
  D27c: ({ C, G, R }) => ({ extra: `border:1px solid var(--hairline);background:color-mix(in srgb,${C} 12%,var(--card-face))`, layers: [L(`position:absolute;inset:5cqmin;border-radius:4cqmin;border:2cqmin solid ${C}`, ''), L(rSans, R), L(pipBR, G), L(wm(0.1), G)] }),
  ...
  D55b: ({ G, R }) => ({ extra: 'border:1px solid var(--hairline)', layers: [L('position:absolute;inset:5cqmin;border-radius:4cqmin;border:3cqmin dashed var(--warn);opacity:.7', ''), L('position:absolute;top:3cqmin;left:3cqmin;width:5cqmin;height:5cqmin;border-radius:50%;background:var(--warn)', ''), L('position:absolute;bottom:3cqmin;right:3cqmin;width:5cqmin;height:5cqmin;border-radius:50%;background:var(--warn)', ''), L(rSerif, R), L(pipBR, G)] }),
...
const JOKER_DECOR: Record<string, (mobile: boolean) => Layer[]> = {
  ...
  AR3: () => [
    L('position:absolute;top:7cqh;right:6cqw;font-size:26cqh;line-height:1;color:#fdf6ec', '★'),
