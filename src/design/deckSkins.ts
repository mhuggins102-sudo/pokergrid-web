...
// RULE: never center a BARE glyph with display:flex. iOS WebKit drops
// the anonymous flex item that bare text creates when the layer sits
// inside a <button> subtree (the SkinStore tiles) — the box renders,
// the glyph vanishes. Center with text-align + line-height (= box
// height, plus any former padding offset) instead. Flex is fine when
// the items are real elements (idx()'s kids below).
...
const wm = (op: number) => `position:absolute;inset:0;text-align:center;font-size:60cqh;line-height:100cqh;opacity:${op};pointer-events:none`;
// Story skins (wanted trio D51a/N1/N2, court EX13, art plates AR1/AR4):
// their type IS the fiction, so they pin Fraunces literally instead of
// following the theme's --font-display (Drafting Room's mono would break
// character). Fraunces is bundled for every theme via main.tsx.
...
const oldTimey = (mobile: boolean): Builder => ({ C, G, R }) => {
  ...
  // would wash the frame out on the cream face). The dominant outer
  // double rule is TWO nested solid rings, not border:double — `double`
  // collapses to a single line below ~3 device px and can drop whole
  // sides at fractional widths, so it only read as double on big cells.
  const frame = [
    L('position:absolute;inset:2.5cqmin;border:0.75cqmin solid color-mix(in srgb,var(--warn) 72%,var(--card-black));border-radius:calc(var(--radius-sm,8px)*0.65)', ''),
    L('position:absolute;inset:3.95cqmin;border:0.75cqmin solid color-mix(in srgb,var(--warn) 72%,var(--card-black));border-radius:calc(var(--radius-sm,8px)*0.45)', ''),
    L('position:absolute;inset:6.5cqmin;border:1cqmin solid color-mix(in srgb,var(--warn) 72%,var(--card-black));border-radius:calc(var(--radius-sm,8px)*0.3);opacity:.5', ''),
  ];
  const center = fig
    ? L(`position:absolute;inset:0;text-align:center;font-size:${mobile ? 68 : 58}cqh;line-height:100cqh;color:${C}`, fig)
    : L(`position:absolute;inset:0;text-align:center;font:680 ${mobile ? 48 : 44}cqh ${FRAUNCES};line-height:100cqh;color:${C}`, R);
  if (mobile) {
    return { extra: 'border:1px solid var(--hairline);background:color-mix(in srgb,var(--warn) 7%,var(--card-face))', layers: [
      ...frame, center,
      ...(fig ? [L(`position:absolute;top:7cqh;left:9cqw;font:680 26cqh ${FRAUNCES};line-height:.9;color:${C}`, R)] : []),
      L(`position:absolute;top:8cqh;right:9cqw;font-size:20cqh;color:${C}`, G),
    ] };
  }
  return { extra: 'border:1px solid var(--hairline);background:color-mix(in srgb,var(--warn) 7%,var(--card-face))', layers: [
    ...frame, center,
    L(`position:absolute;top:8cqh;left:10cqw;font:680 18cqh ${FRAUNCES};line-height:.9;color:${C}`, R),
    L(`position:absolute;bottom:8cqh;right:10cqw;font:680 18cqh ${FRAUNCES};line-height:.9;transform:rotate(180deg);color:${C}`, R),
...
const DESKTOP: Record<string, Builder> = {
  ...
  X2: ({ G, R }) => ({
    ...
    layers: [
      ...
      L("position:absolute;right:6cqw;bottom:6cqh;width:40cqmin;height:40cqmin;border-radius:50%;border:0.5cqmin dashed rgba(234,242,255,.6);text-align:center;font-size:19cqh;line-height:40cqmin;color:#eaf2ff", G),
  ...
  D21: ({ C, G, R }) => ({ extra: 'border:1px solid var(--hairline)', layers: [L(`position:absolute;inset:3cqmin;border-radius:calc(var(--radius-sm,8px)*0.65);border:2.5cqmin solid ${C};opacity:.4`, ''), L(rSans, R), L(pipBR, G)] }),
  ...
  D27c: ({ C, G, R }) => ({ extra: `border:1px solid var(--hairline);background:color-mix(in srgb,${C} 12%,var(--card-face))`, layers: [L(`position:absolute;inset:3cqmin;border-radius:calc(var(--radius-sm,8px)*0.65);border:2cqmin solid ${C}`, ''), L(rSans, R), L(pipBR, G), L(wm(0.1), G)] }),
  ...
  // Filigree's double rule is TWO nested solid rings, not border:double —
  // `double` needs ≥3 device px or it collapses to a single line (and at
  // fractional sub-pixel widths whole sides drop), so grid-size cells
  // rendered it single while the big draw well rendered it double.
  D41c: ({ G, R }) => ({ extra: 'border:1px solid var(--hairline)', layers: [L('position:absolute;inset:3cqmin;border:0.85cqmin solid var(--warn);border-radius:calc(var(--radius-sm,8px)*0.65);opacity:.7', ''), L('position:absolute;inset:4.65cqmin;border:0.85cqmin solid var(--warn);border-radius:calc(var(--radius-sm,8px)*0.45);opacity:.7', ''), L(rSerif, R), L(pipBR, G), L(wm(0.08), G)] }),
  ...
  // Emboss rings: a faint ink rim + stronger inset shadows so the
  // circle reads at grid size — pure embossing only showed when zoomed.
  D42: ({ G, R }) => ({ extra: 'border:1px solid var(--hairline)', layers: [L('position:absolute;right:5cqw;bottom:5cqh;width:54cqmin;height:54cqmin;border-radius:50%;border:1cqmin solid color-mix(in srgb,var(--ink) 20%,transparent);box-shadow:inset 0 2cqmin 3cqmin rgba(255,255,255,.65),inset 0 -2cqmin 3cqmin rgba(0,0,0,.22);text-align:center;font-size:30cqh;line-height:54cqmin', G), L('position:absolute;top:6cqh;left:8cqw;font:700 36cqh var(--font-body);line-height:1;letter-spacing:-.03em', R)] }),
  D42a: ({ C, G, R }) => ({ extra: 'border:1px solid var(--hairline)', layers: [L('position:absolute;right:4cqw;bottom:4cqh;width:58cqmin;height:58cqmin;border-radius:50%;border:1cqmin solid color-mix(in srgb,var(--ink) 20%,transparent);box-shadow:inset 0 2cqmin 3cqmin rgba(255,255,255,.65),inset 0 -2cqmin 3cqmin rgba(0,0,0,.22)', ''), L(`position:absolute;right:calc(4cqw + 8.5cqmin);bottom:calc(4cqh + 8.5cqmin);width:41cqmin;height:41cqmin;border-radius:50%;border:1cqmin solid ${C};opacity:.4`, ''), L('position:absolute;right:4cqw;bottom:4cqh;width:58cqmin;height:58cqmin;text-align:center;font-size:26cqh;line-height:58cqmin', G), L('position:absolute;top:6cqh;left:8cqw;font:700 34cqh var(--font-body);line-height:1;letter-spacing:-.03em', R)] }),
  D51a: ({ G, R }) => ({ extra: `background:var(--card-black);clip-path:${WANTED_CLIP}`, layers: [L(`position:absolute;inset:2cqmin;background:color-mix(in srgb,var(--warn) 12%,var(--card-face));clip-path:${WANTED_CLIP};border-radius:max(0px,calc(var(--radius-sm,8px) - 2cqmin))`), L(`position:absolute;top:5cqh;left:0;right:0;text-align:center;font:700 8cqh ${FRAUNCES};letter-spacing:.14em;text-transform:uppercase;color:var(--ink-3)`, 'Wanted'), L(`position:absolute;inset:0;text-align:center;font:680 44cqh ${FRAUNCES};line-height:106cqh`, R), L('position:absolute;bottom:6cqh;right:9cqw;font-size:24cqh', G)] }),
  N1: ({ G, R }) => ({ extra: `background:var(--card-black);clip-path:${WANTED_CLIP}`, layers: [L(`position:absolute;inset:2cqmin;background:color-mix(in srgb,var(--warn) 12%,var(--card-face));clip-path:${WANTED_CLIP};border-radius:max(0px,calc(var(--radius-sm,8px) - 2cqmin))`), L('position:absolute;inset:5cqmin;border:2cqmin dashed var(--card-black);border-radius:2cqmin;opacity:.55', ''), L(`position:absolute;top:6cqh;left:0;right:0;text-align:center;font:700 8cqh ${FRAUNCES};letter-spacing:.14em;text-transform:uppercase;color:var(--ink-3)`, 'Wanted'), L(`position:absolute;inset:0;text-align:center;font:680 42cqh ${FRAUNCES};line-height:105cqh`, R), L('position:absolute;bottom:7cqh;right:11cqw;font-size:22cqh', G)] }),
  N2: ({ G, R }) => ({ extra: `background:var(--card-black);clip-path:${WANTED_CLIP}`, layers: [L(`position:absolute;inset:2cqmin;background:color-mix(in srgb,var(--warn) 18%,var(--card-face));clip-path:${WANTED_CLIP};border-radius:max(0px,calc(var(--radius-sm,8px) - 2cqmin))`), L('position:absolute;inset:5cqmin;border:2cqmin dashed var(--warn);border-radius:2cqmin;opacity:.7', ''), L(`position:absolute;top:6cqh;left:0;right:0;text-align:center;font:700 8cqh ${FRAUNCES};letter-spacing:.14em;text-transform:uppercase;color:var(--ink-3)`, 'Wanted'), L(`position:absolute;inset:0;text-align:center;font:680 42cqh ${FRAUNCES};line-height:105cqh`, R), L('position:absolute;bottom:7cqh;right:11cqw;font-size:22cqh', G)] }),
  D55b: ({ G, R }) => ({ extra: 'border:1px solid var(--hairline)', layers: [L('position:absolute;inset:3cqmin;border-radius:3cqmin;border:3cqmin dashed var(--warn);opacity:.7', ''), L('position:absolute;top:2cqmin;left:2cqmin;width:5cqmin;height:5cqmin;border-radius:50%;background:var(--warn)', ''), L('position:absolute;bottom:2cqmin;right:2cqmin;width:5cqmin;height:5cqmin;border-radius:50%;background:var(--warn)', ''), L(rSerif, R), L(pipBR, G)] }),
  // Bamboo shortens 10 to "T" — the two-digit rank overpowers the big
  // brushed numeral this skin is built around.
  D62c: ({ C, G, R }) => ({ extra: 'border:1px solid var(--hairline);background:color-mix(in srgb,#f3efe2 75%,var(--card-face))', layers: [L('position:absolute;inset:6cqmin;border:1.5cqmin solid #2f7d4f;border-radius:3cqmin;opacity:.6', ''), L('position:absolute;inset:0;text-align:center;font:680 48cqh var(--font-display);line-height:100cqh;color:#2f7d4f', R === '10' ? 'T' : R), L(`position:absolute;top:8cqh;right:11cqw;font-size:18cqh;color:${C}`, G)] }),
  C2: ({ C, G, R }) => ({ extra: 'border:1px solid var(--hairline);background:color-mix(in srgb,#efe7d2 80%,var(--card-face))', layers: [L('position:absolute;inset:5cqmin;border:1.5cqmin solid #2f7d4f;border-radius:calc(var(--radius-sm,8px)*0.3);opacity:.6', ''), L('position:absolute;inset:0;text-align:center;font:680 46cqh var(--font-display);line-height:100cqh;color:#1e5a3a', R), L(`position:absolute;bottom:7cqh;left:0;right:0;text-align:center;font-size:16cqh;color:${C}`, G)] }),
  ...
  // ── Art (fixed palettes) ──
  AR1: ({ G, R, k, four }) => ({ extra: 'border:1px solid var(--hairline);background:#f5efdd', layers: [L('position:absolute;inset:0;background:repeating-conic-gradient(from -34deg at 50% 122%,rgba(176,138,30,.5) 0 2.6deg,transparent 2.6deg 10.4deg)'), L('position:absolute;inset:0;background:linear-gradient(rgba(245,239,221,.95) 26%,rgba(245,239,221,0) 68%)'), L('position:absolute;inset:4cqmin;border:max(1px,1cqmin) solid #b08a1e;border-radius:2.5cqmin'), L('position:absolute;inset:7cqmin;border:max(0.5px,.4cqmin) solid rgba(176,138,30,.55);border-radius:1.8cqmin'), L(`position:absolute;top:8cqh;left:11cqw;font:680 36cqh ${FRAUNCES};line-height:1;color:${artSuit(k, four)}`, R), L(`position:absolute;bottom:8cqh;right:11cqw;font-size:26cqh;line-height:1;color:${artSuit(k, four)}`, G)] }),
...
const MOBILE: Record<string, Builder> = {
  ...
  // Bamboo: "T" for 10 (matches the desktop tier), and the rank rides
  // 9cqh — high enough that long display-font descenders (Drafting's
  // mono Q) clear the carved frame's bottom rule in every theme.
  D62c: ({ C, G, R }) => ({ extra: 'border:1px solid var(--hairline);background:color-mix(in srgb,#f3efe2 75%,var(--card-face))', layers: [L('position:absolute;inset:6cqmin;border:1.5cqmin solid #2f7d4f;border-radius:3cqmin;opacity:.6', ''), L(`position:absolute;top:8cqh;right:11cqw;font-size:27cqh;color:${C}`, G), L('position:absolute;bottom:9cqh;left:9cqw;font:680 60cqh var(--font-display);line-height:1;color:#2f7d4f', R === '10' ? 'T' : R)] }),
  C2: ({ C, G, R }) => ({ extra: 'border:1px solid var(--hairline);background:color-mix(in srgb,#efe7d2 80%,var(--card-face))', layers: [L('position:absolute;inset:5cqmin;border:1.5cqmin solid #2f7d4f;border-radius:calc(var(--radius-sm,8px)*0.3);opacity:.6', ''), L('position:absolute;top:15cqh;left:0;right:0;text-align:center;font:680 46cqh var(--font-display);line-height:1;color:#1e5a3a', R), L(`position:absolute;bottom:13cqh;left:0;right:0;text-align:center;font-size:28cqh;line-height:1;color:${C}`, G)] }),
  ...
  AR1: ({ G, R, k, four }) => ({ extra: 'border:1px solid var(--hairline);background:#f5efdd', layers: [L('position:absolute;inset:0;background:repeating-conic-gradient(from -34deg at 50% 122%,rgba(176,138,30,.5) 0 2.6deg,transparent 2.6deg 10.4deg)'), L('position:absolute;inset:0;background:linear-gradient(rgba(245,239,221,.95) 26%,rgba(245,239,221,0) 68%)'), L('position:absolute;inset:4cqmin;border:max(1px,1cqmin) solid #b08a1e;border-radius:2.5cqmin'), L(`position:absolute;top:7cqh;left:10cqw;font:680 44cqh ${FRAUNCES};line-height:1;color:${artSuit(k, four)}`, R), L(`position:absolute;bottom:7cqh;right:10cqw;font-size:32cqh;line-height:1;color:${artSuit(k, four)}`, G)] }),
...
// ── Jokers ──
//
// Every skin gets a matching joker: the skin's own dressing (background +
// decorative art layers, harvested from its builder with the suit ink swapped
// to the joker purple — so token-driven washes/frames recolor themselves)
// under a per-skin star/JOKER composition. Skins whose identity lives in the
// rank/pip placement echo it (Big pip's jumbo star, Emboss's star-in-ring,
...
const MUSIC_GLOW: Record<string, string> = {
  ...
  MU2: 'color:#ffe9f2;text-shadow:0 0 3cqmin #ff2975',
  MU3: 'color:#f4f1ff;text-shadow:0 0 3cqmin rgba(255,41,117,.7)',
  MU4: 'color:#fdeffa;text-shadow:0 0 3.5cqmin rgba(255,120,210,.75)',
};
const terrainJoker = (ink: string, glow = ''): Layer[] => [
  jStar(`font-size:38cqh;color:${ink}${glow}`),
  jCap(`bottom:7cqh;color:${ink}${glow}`),
];
const wantedJoker = (): Layer[] => [
  L(`position:absolute;top:5cqh;left:0;right:0;text-align:center;font:700 8cqh ${FRAUNCES};letter-spacing:.14em;text-transform:uppercase;color:var(--ink-3)`, 'Wanted'),
  L('position:absolute;inset:0;text-align:center;font-size:38cqh;line-height:108cqh;color:var(--card-black)', '★'),
  L(`position:absolute;bottom:7cqh;left:0;right:0;text-align:center;font:700 8cqh ${FRAUNCES};letter-spacing:.14em;text-transform:uppercase;color:var(--ink-3)`, 'Joker'),
...
const JOKER_DECOR: Record<string, (mobile: boolean) => Layer[]> = {
  // Twin / Quad: stars take the corner-index posts, wordmark through the middle.
  D05a: () => [
    L('position:absolute;top:5cqh;left:6cqw;font-size:24cqh;line-height:1;color:var(--joker)', '★'),
    L('position:absolute;bottom:5cqh;right:6cqw;font-size:24cqh;line-height:1;transform:rotate(180deg);color:var(--joker)', '★'),
    jCap('top:50%;transform:translateY(-50%);color:var(--joker)'),
  ],
  D06c: () => [
    L('position:absolute;top:5cqh;left:6cqw;font-size:18cqh;line-height:1;color:var(--joker)', '★'),
    L('position:absolute;top:5cqh;right:6cqw;font-size:18cqh;line-height:1;color:var(--joker)', '★'),
    L('position:absolute;bottom:5cqh;left:6cqw;font-size:18cqh;line-height:1;transform:rotate(180deg);color:var(--joker)', '★'),
    L('position:absolute;bottom:5cqh;right:6cqw;font-size:18cqh;line-height:1;transform:rotate(180deg);color:var(--joker)', '★'),
    jCap('top:50%;transform:translateY(-50%);color:var(--joker)'),
  ],
  // Big pip: the jumbo pip IS the star.
  D16: () => [
    L('position:absolute;bottom:4cqh;right:4cqw;font-size:64cqh;line-height:1;opacity:.9;color:var(--joker)', '★'),
    L('position:absolute;top:6cqh;left:8cqw;font:700 13cqh var(--font-body);letter-spacing:.14em;color:var(--joker)', 'JOKER'),
  ],
  // Ticket: star between the notches, caption above the straight bottom cut.
  D24: () => [jStar('font-size:38cqh;color:var(--joker)'), jCap('bottom:8cqh;color:var(--joker)')],
  // Emboss rings: the star sits stamped inside the offset ring (the ring
  // itself survives the art harvest — its pip glyph resolves empty — so the
  // decor only adds the star at the same spot, shadow-free).
  D42: () => [
    L('position:absolute;right:5cqw;bottom:5cqh;width:54cqmin;height:54cqmin;text-align:center;font-size:26cqh;line-height:54cqmin;color:var(--joker)', '★'),
    L('position:absolute;top:6cqh;left:8cqw;font:700 13cqh var(--font-body);letter-spacing:.14em;color:var(--joker)', 'JOKER'),
  ],
  D42a: () => [
    L('position:absolute;right:4cqw;bottom:4cqh;width:58cqmin;height:58cqmin;text-align:center;font-size:22cqh;line-height:58cqmin;color:var(--joker)', '★'),
    L('position:absolute;top:6cqh;left:8cqw;font:700 13cqh var(--font-body);letter-spacing:.14em;color:var(--joker)', 'JOKER'),
  ],
  // Wanted posters: the outlaw is the Joker.
  D51a: wantedJoker,
  N1: wantedJoker,
  N2: wantedJoker,
  ...
  EX13: () => [
    ...
    L(`position:absolute;bottom:9cqh;left:0;right:0;text-align:center;font:680 10cqh ${FRAUNCES};letter-spacing:.14em;text-transform:uppercase;color:var(--ink-3)`, 'Joker'),
  ],
  ...
  L1: () => terrainJoker('#ffe8c2', ';text-shadow:0 0 3cqmin rgba(255,186,8,.9)'),
  D69c: () => terrainJoker('#1d4e6b'),
  // Neon: big dark star on the fluoro wash.
  ...
  // Cosmos: the joker is the brightest star in the sky.
  SP1: () => [
    L('position:absolute;top:6cqh;left:8cqw;font-size:34cqh;line-height:1;color:#ffd479;text-shadow:0 0 4cqmin rgba(255,212,121,.8)', '★'),
    L('position:absolute;bottom:5cqh;left:8cqw;font:700 10cqh var(--font-body);letter-spacing:.14em;color:#e9edff', 'JOKER'),
  ],
  // Art set: each in its own idiom.
  AR1: () => [
    jStar('font-size:34cqh;color:#b08a1e'),
    L(`position:absolute;bottom:8cqh;left:0;right:0;text-align:center;font:680 10cqh ${FRAUNCES};letter-spacing:.14em;text-transform:uppercase;color:#a4243b`, 'Joker'),
  ],
  AR2: () => [
    L('position:absolute;top:6cqh;left:8cqw;font-size:34cqh;line-height:1;color:#1c1a17', '★'),
    L('position:absolute;left:66%;right:0;top:26cqh;text-align:center;font:700 7cqh var(--font-body);letter-spacing:.14em;color:#f8f7f2', 'JOKER'),
  ],
  AR3: () => [
    // Star centered on the red disc via the same disc-congruent box the
    // pip uses.
    L('position:absolute;right:-12cqw;top:-12cqh;width:48cqmin;height:48cqmin;text-align:center;font-size:22cqh;line-height:48cqmin;color:#fdf6ec', '★'),
    L('position:absolute;bottom:4cqh;right:7cqw;font:700 10cqh var(--font-body);letter-spacing:.12em;color:#1c1a17', 'JOKER'),
  ],
  AR4: () => [
    jStar(`font-size:38cqh;color:#5d4c63;text-shadow:0 0 1.2cqmin ${IMPRESSION_HALO}`),
    L(`position:absolute;bottom:6cqh;left:0;right:0;text-align:center;font:540 11cqh ${FRAUNCES};letter-spacing:.14em;color:#5d4c63;text-shadow:0 0 1.2cqmin ${IMPRESSION_HALO}`, 'JOKER'),
  ],
  ...
  X2: () => [
    L('position:absolute;right:6cqw;bottom:6cqh;width:40cqmin;height:40cqmin;text-align:center;font-size:16cqh;line-height:40cqmin;color:#eaf2ff', '★'),
...
/**
 * Build the joker face for a skin. Pure. The skin renders with empty rank
 * and pip, so its decorative layers (frames, rings, patterns, scenery)
 * survive as art with the suit ink resolved to the joker purple; layers
 * carrying fixed text (the Wanted eyebrow) or corner-index kids drop out,
...
export function renderJoker(id: string, opts: RenderOpts = {}): CardFace | null {
  ...
  const art = layers.filter(l => !l.glyph && l.kids.length === 0);
  const decor = (JOKER_DECOR[id] ?? jokerDefault)(!!opts.mobile);
  return { wrap: BASE(extra, 'var(--joker)'), layers: [...art, ...decor] };
}
