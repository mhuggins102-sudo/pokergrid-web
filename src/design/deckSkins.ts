...
const oldTimey = (mobile: boolean): Builder => ({ C, G, R }) => {
  ...
  const center = fig
    ...
    : L(`position:absolute;inset:0;text-align:center;font:680 ${mobile ? 48 : 44}cqh var(--font-display);line-height:100cqh;color:${C}`, R);
  if (mobile) {
...
const DESKTOP: Record<string, Builder> = {
  ...
  D55b: ({ G, R }) => ({ extra: 'border:1px solid var(--hairline)', layers: [L('position:absolute;inset:3cqmin;border-radius:calc(var(--radius-sm,8px)*0.65);border:3cqmin dashed var(--warn);opacity:.7', ''), L('position:absolute;top:2cqmin;left:2cqmin;width:5cqmin;height:5cqmin;border-radius:50%;background:var(--warn)', ''), L('position:absolute;bottom:2cqmin;right:2cqmin;width:5cqmin;height:5cqmin;border-radius:50%;background:var(--warn)', ''), L(rSerif, R), L(pipBR, G)] }),
  ...
  // ── Art (fixed palettes) ──
  AR1: ({ G, R, k, four }) => ({ extra: 'border:1px solid var(--hairline);background:#f5efdd', layers: [L('position:absolute;inset:0;background:repeating-conic-gradient(from -34deg at 50% 122%,rgba(176,138,30,.5) 0 2.6deg,transparent 2.6deg 10.4deg)'), L('position:absolute;inset:0;background:linear-gradient(rgba(245,239,221,.95) 26%,rgba(245,239,221,0) 68%)'), L('position:absolute;inset:4cqmin;border:max(1px,1cqmin) solid #b08a1e;border-radius:2.5cqmin'), L('position:absolute;inset:7cqmin;border:max(0.5px,.4cqmin) solid rgba(176,138,30,.55);border-radius:1.8cqmin'), L(`position:absolute;top:8cqh;left:11cqw;font:680 36cqh var(--font-display);line-height:1;color:${artSuit(k, four)}`, R), L(`position:absolute;bottom:8cqh;right:11cqw;font-size:26cqh;line-height:1;color:${artSuit(k, four)}`, G)] }),
...
const JOKER_DECOR: Record<string, (mobile: boolean) => Layer[]> = {
  ...
  AR1: () => [
    ...
    L('position:absolute;bottom:8cqh;left:0;right:0;text-align:center;font:680 10cqh var(--font-display);letter-spacing:.14em;text-transform:uppercase;color:#a4243b', 'Joker'),
  ],
  AR2: () => [
