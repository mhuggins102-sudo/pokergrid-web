...
export function GameScreen({ onReplay, coach }: GameScreenProps) {
  ...
  // Icon-only Discard glyph — shared by the Split dock's icon column and
  // Double Duty's center-stage Discard/Undo row.
  const trashIcon = (
    <svg
      viewBox="0 0 24 24"
      width="17"
                    ...
                    <div className={styles.dockRow}>
                      ...
                      {dd ? (
                        ...
                        <>
                          ...
                          {flipAction ? (
                            actionBtn(flipAction, styles.classicFlip)
                          ) : (
                            <Button
                              variant="secondary"
                              className={styles.classicFlip}
                              disabled
                            >
                              Flip
                            </Button>
              ...
              <div className={styles.handStack}>
                ...
                <div className={styles.actionStack}>
                  ...
                  {commitAction?.id === 'place' &&
                    (state.doubleDuty ? (
                      // Double Duty: the trash Discard rides beside Place
                      // (directly above the ↺ at the row's end), freeing
                      // the action row below for a wider Flip.
                      <div className={styles.commitRow}>
                        {commitBtn()}
                        <Button
                          variant="secondary"
                  ...
                  {(() => {
                    ...
                    if (state.doubleDuty && commitAction?.id === 'place') {
                      // Double Duty deciding turns: a GRID row so the
                      // suit perk and Flip are exactly equal width on
                      // every phone (a Flip's label swap can't reflow
                      // anything, and a disabled stand-in holds Flip's
                      // spot on non-dual draws) → ↺. Discard rides
                      // beside Place.
                      return (
                        <div
                          className={`${styles.actionRow} ${styles.ddActionRow}`}
                        >
                          {perkAction &&
                            actionBtn(perkAction, styles.perkAmber)}
                          {flipAction ? (
                ...
                {finished ? (
                  ...
                  <div className={styles.dtStage}>
                    ...
                    <NextCardWell
                      ...
                      flight={flight}
                      metaExtra={
                        // Double Duty: Flip lives under the deck caption
                        // — right where the eye is on the dual card — so
                        // the action grid keeps two stable rows
                        // (Place | ↺ over perk | Discard) with no
                        // resizing across draws or suit actions. A
                        // disabled stand-in holds the spot on non-dual
                        // draws.
                        state.doubleDuty
                          ? (() => {
                              ...
                              return flipAction ? (
                                actionBtn(flipAction, styles.dtWellBtn)
                              ) : (
                                <Button
                                  variant="secondary"
                                  className={styles.dtWellBtn}
                                  disabled
                                >
                                  Flip
                                </Button>
                    ...
                    <div className={styles.dtActions}>
                      ...
                      {commitAction?.id === 'cancel' ? (
                        ...
                        // Main column: Place + the suit action (Double
                        // Duty's Flip lives by the deck — see metaExtra).
                        // The icon Discard / Undo column rides beside
                        // them in every variant, hidden while the ♣
                        // bonus modal is up — the draw choice owns the
                        // turn, so they'd only sit behind the scrim
                        // doing nothing — and while a suit action is
                        // targeting (ui.banner; Short Circuit's locked
                        // perks reach here with no Cancel action, so the
                        // cancel-branch above never catches them).
