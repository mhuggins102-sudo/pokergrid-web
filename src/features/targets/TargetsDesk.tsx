...
/*
 * The ≥1024px Targets-Up entry. No Claude-Design mockup exists for
 * this screen — it is composed from the established desktop language
 * instead:
 *
 *  - head: eyebrow / Fraunces headline / right-aligned lede
 *    (FreePlayDesk, ChallengesDesk);
 *  - the ladder: a raised panel (DesktopRails .panel chrome) holding a
 *    horizontal run of level rungs — sunken tiles like the mockups'
 *    stat cells, difficulty-toned level labels like FreePlayDesk's
 *    difficulty cards, ✓-cleared / now / best states echoing
 *    ChallengesDesk's beaten badges and selection borders;
 *  - the Starting/Continuing bar with the primary CTA on the right
 *    (FreePlayDesk's start bar, verbatim geometry);
 *  - two explainer cards reusing the phone page's copy.
 *
 * Below the breakpoint TargetsPage renders the phone card unchanged.
 */
